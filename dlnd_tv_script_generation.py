...
# #     """
# #     # TODO: Implement Function
    bat_num = len(int_text) // (batch_size*seq_length)
    int_text = int_text[:bat_num*batch_size*seq_length]
    int_text = np.array(int_text,dtype=np.int32)
    y = np.zeros_like(int_text)
    y[-1],y[:-1] = int_text[0],int_text[1:]
    result = np.zeros((bat_num,2,batch_size,seq_length))
    for id_b in range(0,bat_num):
        for id_b_s in range(0,batch_size):
            curinput = np.array(int_text[seq_length*(bat_num*id_b_s+id_b): seq_length*(bat_num*id_b_s+id_b+1)])
            curtarget = np.array(y[seq_length*(bat_num*id_b_s+id_b):seq_length*(bat_num*id_b_s+id_b+1)])
            result[id_b][0][id_b_s] = curinput
            result[id_b][1][id_b_s] = curtarget
    return result
"""
DON'T MODIFY ANYTHING IN THIS CELL THAT IS BELOW THIS LINE
"""
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    next_wid = np.argmax(probabilities)
    next_word = int_to_vocab[next_wid]
    return next_word
